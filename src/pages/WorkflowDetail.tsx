...
export default function WorkflowDetail() {
  ...
  useEffect(() => {
    ...
    script.textContent = JSON.stringify({ "@context": "https://schema.org", "@type": "HowTo", name: wf.title, description: wf.description || "", ...(seo.estimated_time ? { totalTime: seo.estimated_time } : {}), step: steps.map((s: any, i: number) => ({ "@type": "HowToStep", position: i + 1, name: s.title, text: s.description || "" })) });
    document.head.appendChild(script);
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 pb-20 md:pb-0">
        <article className="container max-w-4xl py-8">
          ...
          {wf.cover_image_url && (<div className="mb-6 aspect-video overflow-hidden rounded-xl"><img src={wf.cover_image_url} alt={wf.title} className="h-full w-full object-cover" /></div>)}

          <h1 className="text-3xl font-bold leading-tight md:text-4xl" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{wf.title}</h1>
          ...
          {wf.description && (<p className="mt-4 text-lg text-muted-foreground leading-relaxed">{wf.description}</p>)}

          {(seo.problem || seo.solution) && (
            <div className="grid gap-4 md:grid-cols-2 mt-6">
              {seo.problem && (<Card className="border-destructive/20 bg-destructive/5"><CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Target className="h-4 w-4 text-destructive" /> {t("workflow.problem")}</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground leading-relaxed">{seo.problem}</p></CardContent></Card>)}
              {seo.solution && (<Card className="border-primary/20 bg-primary/5"><CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2"><Zap className="h-4 w-4 text-primary" /> {t("workflow.solution")}</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground leading-relaxed">{seo.solution}</p></CardContent></Card>)}
            </div>
          )}

          {(seo.target_audience || seo.prerequisites?.length > 0 || seo.use_cases?.length > 0) && (
            <div className="mt-6 grid gap-4 md:grid-cols-3">
              {seo.target_audience && (<Card><CardContent className="p-4"><div className="flex items-center gap-2 text-sm font-medium mb-1.5"><Users className="h-4 w-4 text-muted-foreground" /> {t("workflow.targetAudience")}</div><p className="text-sm text-muted-foreground">{seo.target_audience}</p></CardContent></Card>)}
