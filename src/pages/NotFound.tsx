...
const NotFound = () => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => { setGlitch(true); setTimeout(() => setGlitch(false), 200); }, 3000);
    return () => clearInterval(timer);
  }, [location.pathname]);

  const suggestions = [
    { label: t("notFound.home"), href: "/", icon: Home },
    { label: t("notFound.exploreTools"), href: "/tools", icon: Search },
    { label: t("notFound.categories"), href: "/categories", icon: Compass },
  ];
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="rounded-2xl border bg-card/80 px-6 py-4 shadow-xl backdrop-blur-sm">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  <code className="rounded bg-muted px-1.5 py-0.5 text-xs font-mono text-primary">{location.pathname}</code> {t("notFound.pathNotFound")}
                </p>
              </div>
            </div>
          </div>
          <p className="mb-6 text-muted-foreground">{t("notFound.suggestion")}</p>
