    ...
    <NodeViewWrapper>
      <div className="my-4 p-4 border-2 border-border rounded-lg bg-card hover:border-primary/50 transition-colors group">
        <div className="flex items-start gap-3">
          {tool.logo_url ? (
            <img src={tool.logo_url} alt={tool.name} className="h-12 w-12 rounded-lg object-cover flex-shrink-0" />
          ) : (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-1">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  <Badge variant="outline" className={`text-[10px] h-4 px-1.5 ${pricing.color}`}>{pricing.label}</Badge>
                  {tool.categories?.name && (
                    <Badge variant="secondary" className="text-[10px] h-4 px-1.5">{tool.categories.name}</Badge>
                  )}
            ...
            {tool.short_description && (
              <p className="text-xs text-muted-foreground line-clamp-2 mt-2">{tool.short_description}</p>
            )}
          </div>
        </div>
      </div>
...
export const ToolBlock = Node.create({
  ...
  parseHTML() {
    return [{ tag: 'div[data-type="tool-block"]' }];
  },
