                ...
                <Card className="h-full transition-all hover:shadow-md hover:-translate-y-0.5 hover:border-primary/20">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      {tool?.logo_url && (
                        <img src={tool.logo_url} alt={tool.name} className="h-6 w-6 rounded object-contain" loading="lazy" />
                      )}
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <span className="text-[11px] text-muted-foreground">{profile?.display_name || "Ẩn danh"}</span>
                      {r.ease_of_use && (
