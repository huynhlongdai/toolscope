...
serve(async (req) => {
  ...
  try {
    ...
    // Check search_rules
    const { data: rules } = await supabase.from("search_rules").select("*").eq("is_active", true);
    ...
    if (rules) {
      for (const rule of rules) {
        let matches = false;
        if (rule.match_type === "exact") matches = normalizedQuery === rule.keyword_pattern.toLowerCase();
        else if (rule.match_type === "contains") matches = normalizedQuery.includes(rule.keyword_pattern.toLowerCase());
    ...
    if (redirectUrl) {
      await supabase.from("search_logs").insert({ query, normalized_query: normalizedQuery, results_count: 0, source: source || "hero" });
    ...
    const toolsSummary = tools?.map((t: any) => ({
      id: t.id, name: t.name, slug: t.slug, desc: t.short_description || t.description?.substring(0, 200),
      pricing: t.pricing_type, rating: t.avg_rating, category: t.categories?.name,
      ai_score: t.ai_scores?.overall_score, recommended: t.ai_scores?.is_recommended, trending: t.is_trending,
    }));
...
Chỉ trả về tools thực sự phù hợp với nhu cầu. Sắp xếp theo mức độ phù hợp. Tối đa 10 kết quả.`
      ...
      tools: [{
        ...
        function: {
          ...
          parameters: {
            type: "object",
            properties: {
              results: { type: "array", items: { type: "object", properties: { id: { type: "string" }, name: { type: "string" }, slug: { type: "string" }, reason: { type: "string" } }, required: ["id", "name", "slug", "reason"] } },
              summary: { type: "string" },
            },
    ...
    if (toolCall?.function?.arguments) { try { searchResults = JSON.parse(toolCall.function.arguments); } catch {} }
    ...
    if (pinnedToolIds.length > 0) {
      ...
      for (const pinnedId of pinnedToolIds) {
        if (!existingIds.has(pinnedId)) {
          ...
          if (pinnedTool) enrichedResults.unshift({ id: pinnedTool.id, name: pinnedTool.name, slug: pinnedTool.slug, reason: "Được đề xuất", tool: pinnedTool, pinned: true });
        }
      }
    }
