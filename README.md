...
- **Tạo tài khoản admin thật**: hiện chỉ có 1 profile placeholder (`admin@toolscope.local`) dùng làm `author_id` cho seed data — cần tạo tài khoản đăng nhập thật qua Supabase Auth (email/password hoặc OAuth) rồi gán role `admin` trong `user_roles` để dùng Admin Dashboard.
...
- **Storage buckets / Edge Functions**: migration đã tạo schema DB đầy đủ, nhưng cần kiểm tra lại các Storage bucket (avatar, tool logo upload...) và 24 Edge Functions gốc có cần deploy riêng lên project mới hay không (migration chỉ chạy SQL, không tự deploy Edge Functions).
