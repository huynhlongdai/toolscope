...
- **2026-09-07 (Agent API Tokens + docs tích hợp cho AI Agent)**: trước đây cách DUY NHẤT cấp quyền cho AI agent là `invite-editor` (gửi email thật, cần người bấm link đặt password) — không dùng được cho agent tự động, và không có nơi nào giải thích cho AI agent cách kết nối (chỉ có comment trong source code, không ai nhìn thấy). Đã bổ sung:
  ...
  - **Chưa làm**: frontend build xong (bao gồm cả trang `/admin/agent-tokens`) nhưng **chưa deploy lên VPS** — VPS `57.155.90.49` không kết nối được từ sandbox trong cả 2 lần thử (cách nhau ~1 giờ), port 22/80/443 đều timeout trong khi google.com/github.com vẫn reachable bình thường từ cùng sandbox → không phải lỗi credentials/SSH key, nhiều khả năng là firewall chặn IP sandbox hoặc VPS đang down. Cần thử lại ở phiên sau hoặc kiểm tra trực tiếp trạng thái VPS/firewall.
