...
- **2026-09-07 (Agent API Tokens + docs tích hợp cho AI Agent)**: trước đây cách DUY NHẤT cấp quyền cho AI agent là `invite-editor` (gửi email thật, cần người bấm link đặt password) — không dùng được cho agent tự động, và không có nơi nào giải thích cho AI agent cách kết nối (chỉ có comment trong source code, không ai nhìn thấy). Đã bổ sung:
  ...
  - **Đã deploy**: migration + 2 edge function (`agent-content-api`, `manage-agent-tokens`) lên Supabase (`yntzlkzckvxlfrrqmmwm`), verify qua curl (token giả trả đúng "Invalid agent token" 401, thiếu auth trả 401 rõ ràng). Build+test local pass (tsc, vitest 26/26). Commit `a1360c8`, đã push GitHub.
  - **Chưa làm**: chưa test tạo token thật + gọi agent-content-api end-to-end bằng token đó (mới verify được nhánh lỗi); frontend build xong nhưng **chưa deploy lên VPS** — VPS `57.155.90.49` không kết nối được từ sandbox lúc thực hiện (port 22/80/443 đều timeout, có thể do firewall/downtime tạm thời), cần thử lại deploy VPS ở phiên sau.
