...
- **Chưa làm**: chưa mở rộng `resource=workflows` (không nằm trong yêu cầu lần này); chưa deploy function mới lên Supabase production (cần Access Token mới, token phiên trước đã hết hạn/không còn lưu); chưa test end-to-end thật cho 3 resource mới (`tools`/`deals`/`translations`) bằng agent token thật — mới verify được build/syntax/tsc/vitest local.
